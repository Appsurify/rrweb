...
export const defaultRecordOptions: recordOptions<RecorderEvent> = {
    ...
    // trigger spurious FullSnapshots during data hydration, bypassing
    // NavigationManager's same-URL coalescing. Page SEGMENTATION does not
    // depend on it (checkoutId is unique across page loads) — NavigationManager
    // alone handles route-change snapshots. Each checkout re-serializes the
    // whole DOM (~1.2 MB with inlined CSS/images), so on mutation-heavy sites
    // the extra snapshots pushed reports past V8's 512 MB max string length
    // and JSON.stringify threw RangeError.
